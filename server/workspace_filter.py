        ...
        if raw:
            ...
            try:
                with open(_SETTINGS_FILE) as f:
                    data = json.load(f)
                ids = [str(i) for i in data.get("workspace_ids", []) if str(i).lstrip("-").isdigit()]
                _cached_ids = ids
...
def build_ws_filter_clause(
    ...
    if id_list is not None:
        valid = [i for i in id_list if i.lstrip("-").isdigit()]
        if not valid:
            return ""
        if len(valid) == 1:
            return f"AND CAST({col} AS BIGINT) = {valid[0]}"
        return f"AND CAST({col} AS BIGINT) IN ({', '.join(valid)})"
