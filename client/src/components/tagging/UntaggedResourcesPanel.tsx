...
import { type UntaggedTab, formatCurrency, formatNumber } from "./shared";
...
interface UntaggedResourcesPanelProps {
  untagged: TaggingDashboardBundle["untagged"];
...
export function UntaggedResourcesPanel({
  untagged,
  untaggedSpend,
  suggestedTags,
  host,
}: UntaggedResourcesPanelProps) {
  ...
  const untaggedCounts = useMemo(
    () => ({
      clusters: untagged.clusters?.count || 0,
      jobs: untagged.jobs?.count || 0,
      pipelines: untagged.pipelines?.count || 0,
      warehouses: untagged.warehouses?.count || 0,
      endpoints: untagged.endpoints?.count || 0,
    }),
    [untagged]
  );
  ...
  const getItems = () => {
    switch (activeUntaggedTab) {
      case "clusters":
        return untagged.clusters?.items || [];
      case "jobs":
        return untagged.jobs?.items || [];
      case "pipelines":
        return untagged.pipelines?.items || [];
      case "warehouses":
        return untagged.warehouses?.items || [];
      case "endpoints":
        return untagged.endpoints?.items || [];
      default:
        return [];
    }
  };
  ...
  const activeItems = showHistoricalUntagged
    ? allItems
    : allItems.filter((item) => !isHistoricalItem(item));
  ...
  return (
    <div className="rounded-lg border bg-white p-6" style={{ borderColor: "#E5E5E5" }}>
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-sm text-red-600">{formatCurrency(untaggedSpend)} untagged spend</span>
      </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <button
                ...
                onClick={() => handleSuggestedTagsMinimize(!suggestedTagsMinimized)}
              >
                <p className="text-sm font-medium text-orange-800">Suggested Tags for Your Environment</p>
                <svg
                  className={`h-4 w-4 text-orange-500 transition-transform ${suggestedTagsMinimized ? "" : "rotate-180"}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {suggestedTags.map((tag) => (
                      <div key={tag.key} className="group relative">
                        <span className="inline-flex cursor-help items-center rounded-full bg-orange-100 px-3 py-1 text-sm font-medium text-orange-800">
                          {tag.key}
                        </span>
                        <div className="invisible absolute bottom-full left-0 z-10 mb-2 w-64 rounded-lg bg-gray-900 p-3 text-xs text-white opacity-0 shadow-xl transition-all group-hover:visible group-hover:opacity-100">
                          <p className="mb-1 font-semibold text-orange-300">{tag.key}</p>
                          <p className="mb-1 text-gray-300">Used by {tag.usageCount} resources</p>
                          {tag.examples.length > 0 && (
                            <div>
                              <p className="text-[10px] uppercase text-gray-500">Example values:</p>
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        <label className="flex cursor-pointer items-center gap-1.5 whitespace-nowrap text-xs text-gray-500">
          <input
            type="checkbox"
            checked={showHistoricalUntagged}
          ...
          <span className="group relative ml-0.5">
            <svg className="inline h-3 w-3 cursor-help text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span className="absolute bottom-full right-0 z-20 mb-1 hidden w-56 rounded-lg bg-gray-900 px-2 py-1.5 text-[10px] text-white shadow-lg group-hover:block">
              Resources whose names could not be resolved — likely deleted or from inaccessible workspaces
            </span>
          </span>
        </label>
      </div>
                    ...
                    {extraColumns.map((col) => (
                      <td key={col.key} className="px-6 py-4 text-sm text-gray-600">
                        {item[col.key] ? (
                          col.key === "owner" ? (
                            <span
                              className="inline-flex max-w-40 truncate rounded-full bg-gray-100 px-2 py-0.5 text-xs font-medium text-gray-700"
                              title={item[col.key]}
                            >
                              {formatIdentity(item[col.key])}
                            </span>
                          ) : (
                            <span className="block max-w-40 truncate" title={item[col.key]}>{item[col.key]}</span>
                          )
                        ) : (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {(() => {
                          ...
                          return (tagMap[activeUntaggedTab] || ["team", "environment", "project"]).map((tag) => (
                            <span key={tag} className="inline-flex items-center rounded bg-gray-100 px-2 py-0.5 text-xs text-gray-600">
                              {tag}
                            </span>
                          ));
                        })()}
          ...
          {totalPages > 1 && (
            <div className="mt-4 flex items-center justify-between border-t border-gray-200 pt-4">
              <p className="text-sm text-gray-700">
                ...
                <span className="font-medium">{Math.min(startIndex + ITEMS_PER_PAGE, sortedItems.length)}</span> of{" "}
